 ...
 */
 ...
 use XBCommon;
 class XiaoeController extends BaseController{

     const T_TABLE='renzen_pdscore';
     ...
     public function DataList(){
         ...
         if($array['rows']){
             foreach ($array['rows'] as $val) {
                 ...
                 $val['Response'] = json_decode($val['Response'],true)['score'];
                 $meminfo='';
     ...
     public function detail(){
        $ID=I('request.ID');
        if($ID){
            $infos=M(self::T_TABLE)->field('ID,UserID,QQ,WeChat,Contents,Status,RenzTime')->find($ID);
            if($infos['Contents']){
                $infos['Contents']=unserialize($infos['Contents']);
                $UserID = $infos['UserID'];
                foreach ($infos['Contents']['qinshu'] as $index => &$people) {
                    $people['callTimes'] = $this->callPhoneHistory($UserID, $people['tel']);
                }
                foreach ($infos['Contents']['shehui'] as $index => &$people2) {
                    $people2['callTimes'] = $this->callPhoneHistory($UserID, $people2['tel']);
                }
