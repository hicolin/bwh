 ...
 */
 ...
 use XBCommon;
 class JpfkController extends BaseController{

     const T_TABLE='renzen_zhima';
     const T_ADMIN='sys_administrator';
     const T_MEMINFO='mem_info';
     const T_SHLIST='renzen_shlist';

     public function index(){
         echo '<div style="color:red;margin-top: 50px;text-align: center">此功能正在研发，敬请期待</div>';exit;
         $this->display();
     }
     ...
     public function DataList(){
         ...
         $where['IsDel']=0;
         //查询的数据表字段名
         $col='ID,UserID,Score as Zmf,UpdatedTime as RenzTime,Status';//默认全字段查询
     ...
     public function detail(){
        ...
        if($ID){
            $infos=M(self::T_TABLE)->alias('a')
                   ->field('a.ID,a.UserID,a.Score as Zmf,a.WatchList as Hygz,a.Status,b.Mobile,b.TrueName')
                   ->join('left join xb_mem_info b on a.UserID=b.ID')
                   ->where(array('a.ID'=>$ID))->find();
            if($infos['Hygz']){
                $infos['Hygz']= json_encode( unserialize($infos['Hygz']) );
                $infos['Hygz'] = json_decode($infos['Hygz'], true);
            }
