        ...
        //不是管理员的话，只能看到自己的单子
         ...
         if($array['rows']){
             foreach ($array['rows'] as $val) {
                 ...
                 $val['Tgadmin']=$query->GetValue('tg_admin',array('ID'=>(int)$meminfo['TgadminID']),'Name');
    ...
    public function aduitsave(){
        ...
        if($result){
            ...
            if($Status=='1'){
                $applydata=array();
                if($mark==1){
                    $applydata['UpdateTime']=date('Y-m-d H:i:s');
                }else{
            ...
            if($mark==1 && $hkinfos['LoanNo']==$applyinfos['LoanNo']){
                $xjdata=array(
                    'UserID'=>$hkinfos['UserID'],
                    'ApplyID'=>$hkinfos['ApplyID'],
                    'OrderSn'=>date(ymd).rand(1,9).date(His).rand(111,999),
                    'LoanNo'=>$applyinfos['LoanNo'],
                    'TradeNo'=>'',
                    'LoanDay'=>'7',
//                    'TotalMoney'=>$applyinfos['ApplyMoney'],
                    'TotalMoney'=>'',
                    'ServiceCost'=>$hkinfos['TotalMoney'],
                ...
                M('loans_xjapplylist')->add($xjdata);
            }
